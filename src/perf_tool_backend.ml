...
module Recording = struct
  ...
  let perf_args_of_collection_mode
    ...
    in
    let extra_events =
      perf_config_of_extra_events ~selector collection_mode.extra_events
    in
  ...
  let attach_and_record
    ...
    (* CR-someday alamoreaux: [--per-thread] is an important argument here.
       ...
       trace better. *)
    let per_thread_opts =
      match collection_mode.extra_events with
      | [] -> [ "--per-thread" ]
      | _ -> []
    in
    let thread_opts =
    ...
    let snapshot_size_opt =
      match snapshot_size, collection_mode.primary with
    ...
    in
    let overwrite_opts =
      match collection_mode.primary, full_execution with
      | Stacktrace_sampling, false -> [ "--overwrite" ]
      | Intel_processor_trace, false | _, true -> []
    in
    let switch_opts =
