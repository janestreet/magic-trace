...
let perf_event_header_re =
  Re.Perl.re
    {|^ *([0-9]+)/([0-9]+) +([0-9]+)\.([0-9]+): +(([0-9]+) )? *([a-z\-]+)(/[a-z=0-9]+)?(/[a-z]*)?:([a-zA-Z]+:)?(.*)$|}
  |> Re.compile
;;
...
let parse_callstack_entry ?perf_maps (thread : Event.Thread.t) line : Event.Location.t =
  ...
  | [| _str; instruction_pointer; symbol_and_offset |] ->
...
let%test_module _ =
  (module struct
    ...
    let%expect_test "C symbol" =
      check
        {| 25375/25375 4509191.343298468:                                branches:uH:   call                     7f6fce0b71f4 __clock_gettime+0x24 (foo.so) =>     7ffd193838e0 __vdso_clock_gettime+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
    ...
    let%expect_test "C symbol trace start" =
      check
        {| 25375/25375 4509191.343298468:                                branches:uH:   tr strt                             0 [unknown] (foo.so) =>     7f6fce0b71d0 __clock_gettime+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
          ((thread ((pid (25375)) (tid (25375)))) (time 52d4h33m11.343298468s)
    ...
    let%expect_test "C++ symbol" =
      check
        {| 7166/7166  4512623.871133092:                                branches:uH:   call                           9bc6db a::B<a::C, a::D<a::E>, a::F, a::F, G::H, a::I>::run+0x1eb (foo.so) =>           9f68b0 J::K<int, std::string>+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
    ...
    let%expect_test "OCaml symbol" =
      check
        {|2017001/2017001 761439.053336670:                                branches:uH:   call                     56234f77576b Base.Comparable.=_2352+0xb (foo.so) =>     56234f4bc7a0 caml_apply2+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
    ...
    let%expect_test "manufactured example 1" =
      check
        {|2017001/2017001 761439.053336670:                                branches:uH:   call                     56234f77576b x => +0xb (foo.so) =>     56234f4bc7a0 caml_apply2+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
          ((thread ((pid (2017001)) (tid (2017001)))) (time 8d19h30m39.05333667s)
    ...
    let%expect_test "manufactured example 2" =
      check
        {|2017001/2017001 761439.053336670:                                branches:uH:   call                     56234f77576b x => +0xb (foo.so) =>     56234f4bc7a0 => +0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
          ((thread ((pid (2017001)) (tid (2017001)))) (time 8d19h30m39.05333667s)
    ...
    let%expect_test "manufactured example 3" =
      check
        {|2017001/2017001 761439.053336670:                                branches:uH:   call                     56234f77576b + +0xb (foo.so) =>     56234f4bc7a0 caml_apply2+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
          ((thread ((pid (2017001)) (tid (2017001)))) (time 8d19h30m39.05333667s)
    ...
    let%expect_test "unknown symbol in DSO" =
      check
        {|2017001/2017001 761439.053336670:                                branches:uH:   call                     56234f77576b [unknown] (foo.so) =>     56234f4bc7a0 caml_apply2+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
          ((thread ((pid (2017001)) (tid (2017001)))) (time 8d19h30m39.05333667s)
    ...
    let%expect_test "DSO with spaces in it" =
      check
        {|2017001/2017001 761439.053336670:                                branches:uH:   call                     56234f77576b [unknown] (this is a spaced dso.so) =>     56234f4bc7a0 caml_apply2+0x0 (foo.so)|};
      [%expect
        {|
        ((Ok
          ((thread ((pid (2017001)) (tid (2017001)))) (time 8d19h30m39.05333667s)
    ...
    let%expect_test "power event psb offs" =
      check
        "2937048/2937048 448556.689403475:                                        \
         psb:                        psb offs: 0x4be8                                \
    ...
    let%expect_test "sampled callstack" =
      check
        "2060126/2060126 178090.391624068: cycles:u:\n\
