...
let write_event (t : t) event =
  ...
  | None, Some Start ->
    ...
    then
      ...
      ret_track_exn_data ()
  | Some Syscall, None | Some Hardware_interrupt, None ->
    ...
    ; (if [%compare.equal: Event.Kind.t option] kind (Some Hardware_interrupt)
      then [ Kernel ]
      else [])
    ]
    |> List.concat
